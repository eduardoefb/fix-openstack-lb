...
pub fn update_pool_status_to_active(pool_id: &str) -> Result<(), Box<dyn Error>> {
...
pub fn update_pool_status_to_pending(pool_id: &str) -> Result<(), Box<dyn Error>> {
    let conf = Ini::load_from_file("/etc/octavia/octavia.conf").unwrap();
    let db_section = conf.section(Some("database")).unwrap();
    let conn_str = db_section.get("connection").unwrap().replace("mysql+pymysql", "mysql");
    ...
    // Execute the SQL query
    conn.exec_drop(        
        r"UPDATE pool SET provisioning_status='PENDING_CREATE' WHERE provisioning_status='ACTIVE' AND id=:pool_id",
        params! {
            "pool_id" => pool_id,
        },
    )
